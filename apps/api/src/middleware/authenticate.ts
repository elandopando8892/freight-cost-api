...
import { verifyKindeToken, resolveUser } from '../modules/auth/kinde.service.js'
...
export async function authenticate(request: FastifyRequest, reply: FastifyReply) {
  ...
  try {
    ...
    request.user = { sub: user.id, orgId: user.orgId, role: user.role, kindeId: user.kindeId } as JwtPayload
  } catch (err) {
